-- Migration 023: Fix handle_new_user() trigger — add missing email column
-- ============================================================================
-- The profiles table has email VARCHAR(255) NOT NULL, but the trigger
-- didn't include email in its INSERT, causing silent failure (caught by
-- EXCEPTION WHEN OTHERS). This broke the entire invite flow because:
--   1. No profile created → 406 on profile queries
--   2. household_members.user_id REFERENCES profiles(id) → FK violation
-- ============================================================================
...
BEGIN
    ...
    INSERT INTO public.profiles (id, email, display_name, avatar_url, username, setup_completed, updated_at)
    VALUES (
        NEW.id,
        NEW.email,
        v_display_name,
