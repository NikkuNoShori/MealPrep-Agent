-- Migration 018: Add secure invite tokens + auto-join on signup
-- ============================================================================
-- 1. Add token_hash column to household_invites for secure token validation
-- 2. Update handle_new_user() to auto-accept pending invites on signup
-- ============================================================================

-- 1. Add token_hash column and index
...
CREATE INDEX IF NOT EXISTS idx_household_invites_token_hash
  ON household_invites(token_hash) WHERE token_hash IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_household_invites_email_pending
  ON household_invites(invited_email, status) WHERE status = 'pending';

-- 2. Update handle_new_user() to auto-join households from pending invites
